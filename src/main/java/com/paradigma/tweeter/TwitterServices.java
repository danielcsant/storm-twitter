...
public class TwitterServices {
    ...
    // Mentions Timeline
    private static final String TWITTER_MENTIONS_TIMELINE_ENDPOINT = "https://api.twitter.com/1.1/statuses/mentions_timeline.json";
    private static final String TWITTER_MENTIONS_TIMELINE_ENDPOINT_PATH = "/1.1/statuses/mentions_timeline.json";
...
//		paramMap.put(OAH_SIGNATURE, signature);
	...
	public static void statusUpdate(String status, String oauthToken) throws TwitterException {
		...
		// signature parameters
		HashMap<String, String> signatureParameters = new HashMap<String, String>();
		signatureParameters.put(OAH_CONSUMER_KEY, TwitterCredentials.TWITTER_SERVICES_CONSUMER_KEY);
		signatureParameters.put(OAH_NONCE, _numberUsedOnce);
		signatureParameters.put(OAH_TIMESTAMP, _timeStamp);
		// include access token
		signatureParameters.put(OAH_TOKEN, oauthToken);
		...
		try {
			oauthSignature = generateOauthSignature(signatureParameters, TWITTER_STATUSES_UPDATE_ENDPOINT,
					TwitterCredentials.TWITTER_SERVICES_CONSUMER_SECRET, TwitterCredentials.TWITTER_SERVICES_ACCESS_TOKEN_SECRET,
					METHOD_POST);
		} catch (Exception e) {
			throw new TwitterException(e);
		}
		...
		// make the HTTP request
		String responseBody = makeHttpRequest(TWITTER_ENDPOINT_HOST, TWITTER_STATUSES_UPDATE_ENDPOINT_PATH,
				authHeader, requestBody, METHOD_POST);
		logger.info(responseBody);
	}

    public static void getMentions() throws TwitterException {
        // generate values for this request
        String _numberUsedOnce = generateNumberUsedOnce();
        String _timeStamp = currentTimeInSeconds();
        ...
        String oauthSignature = null;
        try {
            oauthSignature = generateOauthSignature(signatureParameters, TWITTER_MENTIONS_TIMELINE_ENDPOINT,
                    TwitterCredentials.TWITTER_SERVICES_CONSUMER_SECRET, TwitterCredentials.TWITTER_SERVICES_ACCESS_TOKEN_SECRET,
                    METHOD_GET);
        } catch (Exception e) {
            throw new TwitterException(e);
...
//        bodyParameters.put(TWITTER_STATUSES_UPDATE_KEY, status);
	 ...
	 * retrieve user info
	 */
	public static void userShow (String screenName) throws TwitterException {
		...
		try {
			oauthSignature = generateOauthSignature(signatureParameters, TWITTER_USERS_SHOW_ENDPOINT,
					TwitterCredentials.TWITTER_SERVICES_CONSUMER_SECRET, 
					TwitterCredentials.TWITTER_SERVICES_ACCESS_TOKEN_SECRET, METHOD_GET);
		} catch (Exception e) {
			throw new TwitterException(e);
		}
		...
		StringBuffer endPointPath = new StringBuffer();
		...
		String responseBody = makeHttpRequest(TWITTER_ENDPOINT_HOST, endPointPath.toString(),
				authHeader, "", METHOD_GET);
